...
console.log('****', this.getESIndexName(changeStreamObj.ns.coll));
  ...
  async deleteElasticCollection(collectionName) {
    ...
    let numDeleted = 0;
    let hitsTotal = (typeof searchResponse.hits.total === 'object') ? searchResponse.hits.total.value : searchResponse.hits.total;
    for (let i = 0; i < Math.ceil(hitsTotal / this.bulkSize); i++) {
      const bulkDelete = [];
      ...
      logger.info(`${collectionName} delete progress: ${numDeleted}/${hitsTotal}`);
